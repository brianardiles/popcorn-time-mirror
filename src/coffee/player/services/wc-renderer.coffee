...
.factory 'wcjsRenderer', (wcAddon) ->
  Texture = (gl, width, height) ->
    ...
    gl.texParameteri gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGEn

  Texture::bind = (n, program, name) ->
    gl = @gl
  ...
  setupCanvas = (canvas, vlc, fallbackRenderer) ->
    ...
    # Comment this line out to test fallback
    gl = canvas.gl
  ...
  init: (canvas, params, fallbackRenderer) ->
    ...
    if typeof params != 'undefined'
      ...
      vlc = wcAddon.createPlayer()
    
    if typeof canvas == 'string'
      setupCanvas window.document.querySelector(canvas), vlc, fallbackRenderer
    else
      setupCanvas canvas, vlc, fallbackRenderer
