    ...
    <div class="content">
        <div class="onlinesearch">
            <div class="dropdown online-categories">
                <% var arr_categories=[ "Movies", "TV Series", "Anime"]; var select_category="" ; for(var key in arr_categories) { select_category +="<option " +(Settings.OnlineSearchCategory==arr_categories[key]? "selected='selected'": "")+ " value='"+arr_categories[key]+ "'>"+i18n.__(arr_categories[key])+ "</option>"; } %>
                    <select name="online-category">
                        <%=select_category%>
                    </select>
            ...
            <ul class="file-list">
                <% _.each(fs.readdirSync(require( 'nw.gui').App.dataPath + '/TorrentCollection/'), function(file, index) { %>
                    <li class="file-item" data-index="<%=file.index%>" data-file="<%=index%>">
                        ...
                        <% if (file.indexOf( '.torrent') !==- 1) { var icon="torrent-icon" ; } else { var icon="magnet-icon" ; } %>
