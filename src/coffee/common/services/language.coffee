...
.constant 'allTranslations', 
  [
    ...
    'sl', 'sr', 'sv', 'tr', 'zh-cn', 'zh-tw'
  ]

.factory 'language', (langcodes, allTranslations) ->
  # Detect the language and update the global Language file
  setLanguage = (preferredLanguage) ->
    if !preferredLanguage
      # we are still on default
      lang = App.Localization.detectLocale()
      i18n.setLocale lang
      AdvSettings.set 'language', lang
    else i18n.setLocale preferredLanguage
  ...
  detectLocale = ->
    ...
    baseLanguage = navigator.language.toLowerCase().slice(0, 2)
    
    if pureLanguage in allTranslations
      pureLanguage
    else if baseLanguage in allTranslations
      baseLanguage
    else 'en'

  # Remove unsupported subtitle language from object
  filterSubtitle = (langs) ->
    filteredLang = {}

    angular.forEach langs, (data, lang) ->
      langInfo = langcodes[lang]

      if langInfo and langInfo.subtitle
